...
export interface BrandNodeProps extends SharedNodeProps {}
    ...
    <>
      ...
      >
        ...
        {node.branches?.map((branch, index) => (
          <div
            key={branch.id}
            className="relative flex flex-col items-center before:absolute before:top-0 before:left-1/2 before:h-full before:w-[2px] before:-translate-x-1/2 before:bg-stone-200 before:content-['']"
          >
            {index == 0 && (
              <>
                <div className="absolute -top-1 -left-[1px] h-2 w-1/2" style={{ backgroundColor: themeToken.colorBgContainer }}></div>
            ...
            {index == node.branches!.length - 1 && (
              <>
                <div className="absolute -top-1 -right-[1px] h-2 w-1/2" style={{ backgroundColor: themeToken.colorBgContainer }}></div>
                <div className="absolute -right-[1px] -bottom-1 z-50 h-2 w-1/2" style={{ backgroundColor: themeToken.colorBgContainer }}></div>
              </>
            )}
